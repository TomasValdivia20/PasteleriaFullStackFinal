# =====================================================================
# SCRIPT DE VERIFICACION COMPLETA - RAILWAY & VERCEL
# Verifica que Backend retorne variantes correctamente
# =====================================================================

Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "VERIFICACION COMPLETA - VARIANTES" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "   OK - Respuesta recibida correctamente`n" -ForegroundColor Green
    ...
    # VERIFICACION CRITICA: Variantes
    Write-Host "`n   VARIANTES:" -ForegroundColor Magenta
    
    if ($response.variantes -and $response.variantes.Count -gt 0) {
        Write-Host "      OK - Total variantes: $($response.variantes.Count)" -ForegroundColor Green
        ...
        Write-Host "`n   EXITO: Backend retorna variantes correctamente" -ForegroundColor Green
        $backendOk = $true
    } else {
        Write-Host "      ERROR: No hay variantes" -ForegroundColor Red
        Write-Host "      Variantes array: $($response.variantes)" -ForegroundColor Yellow
        Write-Host "`n   ACCION REQUERIDA: Ejecutar SQL_INSERT_VARIANTES_PRODUCTOS.sql en Supabase" -ForegroundColor Yellow
        $backendOk = $false
    }
    
    # Verificar imagenes
    Write-Host "`n   Imagenes:" -ForegroundColor Cyan
    if ($response.imagenes -and $response.imagenes.Count -gt 0) {
        Write-Host "      OK - Total imagenes: $($response.imagenes.Count)" -ForegroundColor Green
    } else {
        Write-Host "      INFO - Sin imagenes adicionales (usando imagen principal)" -ForegroundColor Gray
    }
...
try {
    ...
    if ($health.status -eq "UP") {
        ...
        Write-Host "   WARN - Health Check: $($health.status)" -ForegroundColor Yellow
        $healthOk = $false
    }
} catch {
    Write-Host "   WARN - Health endpoint no disponible (puede ser normal)" -ForegroundColor Yellow
    $healthOk = $null
}
...
try {
    ...
    Write-Host "   OK - Total productos: $($productos.Count)" -ForegroundColor Green
    ...
    if ($totalVariantes -eq 58) {
        ...
        Write-Host "`n   ADVERTENCIA: Se encontraron $totalVariantes variantes, se esperaban 58" -ForegroundColor Yellow
    } else {
        Write-Host "`n   ERROR: No se encontraron variantes" -ForegroundColor Red
    }
    
    # Mostrar productos con mas variantes
...
if ($backendOk) {
    Write-Host "OK - Backend Railway: OPERATIVO" -ForegroundColor Green
    Write-Host "OK - Variantes: FUNCIONANDO ($($response.variantes.Count) variantes en producto 1)" -ForegroundColor Green
} else {
    Write-Host "ERROR - Backend Railway: CON PROBLEMAS" -ForegroundColor Red
    Write-Host "ERROR - Variantes: NO DISPONIBLES" -ForegroundColor Red
}

if ($healthOk -eq $true) {
    Write-Host "OK - Health Check: UP" -ForegroundColor Green
} elseif ($healthOk -eq $false) {
    Write-Host "WARN - Health Check: DOWN" -ForegroundColor Yellow
} else {
    Write-Host "INFO - Health Check: No verificado" -ForegroundColor Gray
}
